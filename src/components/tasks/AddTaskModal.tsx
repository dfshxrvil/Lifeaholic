...
export function AddTaskModal({ visible, date, onClose, onAdd, task }: { task?: Task | null; visible: boolean; date: string; onClose: () => void; onAdd: (title: string, description: string | undefined, priority: TaskPriority) => Promise<void> }) {
  ...
  useEffect(() => { if (visible) { setTitle(task?.title ?? ''); setDescription(task?.description ?? ''); setPriority(task?.priority ?? 'red'); setError(null); } }, [visible, task]);
  const submit = async () => { if (!title.trim()) { fail('Give your task a title.'); return; } setLoading(true); setError(null); try { await onAdd(title.trim(), description.trim() || undefined, priority); onClose(); } catch (cause) { fail(cause instanceof Error ? cause.message : 'Unable to save task.'); } finally { setLoading(false); } };
  return <AppModal visible={visible} onClose={() => { if (!loading) onClose(); }}><ValidationFeedback trigger={errorPulse}><View style={styles.content}>
    <View><Text style={[styles.title, { color: colors.text }]}>{task ? 'Edit task' : 'New task'}</Text><Text style={[styles.subtitle, { color: colors.textMuted }]}>{new Date(`${date}T12:00:00`).toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' })}</Text></View>
