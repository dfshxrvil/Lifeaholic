...
import { Keyboard, LayoutAnimation, Platform, ScrollView, StyleSheet, Text, TextInput, View } from 'react-native';
...
import { formatExpenseDate, formatExpenseTime, mergeExpenseTimestamp } from '@/utils/expenseTimestamp';
...
export function AddExpenseModal({ visible, groups, expense, onClose, onSaved }: Props) {
  ...
  const [selectedTimestamp, setSelectedTimestamp] = useState(() => new Date()); const [groupId, setGroupId] = useState<FinanceId | null>(null);
  ...
  const [pickerMode, setPickerMode] = useState<ExpensePickerMode | null>(null); const [localError, setLocalError] = useState<string | null>(null); const [errorPulse, setErrorPulse] = useState(0);
  const roster = useGroupRoster(groupId); const members = roster.members.filter((member) => member.status === 'active');
  const debugError = formatDebugError('EXPENSE MUTATION ERROR', mutation.errorDetails)
    ?? formatDebugError('GROUP ROSTER ERROR', roster.errorDetails);

  useEffect(() => {
    if (!visible) return;
    beginAction(); setDescription(expense?.description ?? ''); setAmount(expense ? paiseToRupeeInput(expense.totalAmountMinor) : '');
    setCategory(expense?.category ?? 'Food'); setCustomCategoryNote(expense?.customCategoryNote ?? ''); setSelectedTimestamp(expense ? new Date(expense.transactionTimestamp ?? `${expense.expenseDate}T00:00:00`) : new Date()); setGroupId(expense?.groupId ?? null);
    ...
    setPanel(null); setPickerMode(null); setLocalError(null);
  }, [beginAction, expense, user?.id, visible]);

  const fail = (message: string) => { setLocalError(message); setErrorPulse((value) => value + 1); };
  const openPicker = (mode: ExpensePickerMode) => { Keyboard.dismiss(); setPickerMode(mode); };
  const selectTimestamp = (picked: Date) => {
    if (!pickerMode) return;
    const next = mergeExpenseTimestamp(selectedTimestamp, picked, pickerMode);
    if (!Number.isFinite(next.getTime()) || next.getTime() > Date.now()) {
      fail('Choose a date and time that is not in the future.');
      return;
    }
  ...
  const submit = async () => {
    // eslint-disable-next-line react-hooks/purity -- Checked when the user presses Save, never during render.
    if (selectedTimestamp.getTime() > Date.now()) return fail('Choose a date and time that is not in the future.');
    ...
    try {
      ...
      const draft = { groupId, description: description.trim(), totalAmountMinor: total, category, customCategoryNote: customCategoryNote.trim() || null, expenseDate: toDateKey(selectedTimestamp), transactionTimestamp: selectedTimestamp.toISOString(), participants };
  ...
  return <><AppModal visible={visible} onClose={mutation.loading ? () => {} : onClose}><ValidationFeedback trigger={errorPulse}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.content}>
    ...
    <View style={styles.controlBar}>
      <AnimatedPressable accessibilityRole="button" accessibilityLabel={`Choose expense date, ${formatExpenseDate(selectedTimestamp)}`} disabled={mutation.loading} onPress={() => openPicker('date')} style={[styles.iconButton, { backgroundColor: colors.accentSoft, borderColor: colors.border }]}><Calendar size={18} color={colors.accent} /><Text style={[styles.controlText, { color: colors.text }]}>{formatExpenseDate(selectedTimestamp)}</Text></AnimatedPressable>
      <AnimatedPressable accessibilityRole="button" accessibilityLabel={`Choose expense time, ${formatExpenseTime(selectedTimestamp)}`} disabled={mutation.loading} onPress={() => openPicker('time')} style={[styles.iconButton, { backgroundColor: colors.accentSoft, borderColor: colors.border }]}><Clock size={18} color={colors.accent} /><Text style={[styles.controlText, { color: colors.text }]}>{formatExpenseTime(selectedTimestamp)}</Text></AnimatedPressable>
    </View>
    {visible && pickerMode && <View style={styles.picker}>
      <ExpenseDateTimePicker value={selectedTimestamp} mode={pickerMode} maximumDate={new Date()} theme={theme === 'light' ? 'light' : 'dark'} onSelect={selectTimestamp} onDismiss={() => setPickerMode(null)} />
      {Platform.OS !== 'android' && <Button label="Done" variant="secondary" onPress={() => setPickerMode(null)} />}
    </View>}
    {expense && !expense.transactionTimestamp && <Text style={[styles.status, { color: colors.textMuted }]}>The original time was not recorded. Choose a time before saving.</Text>}
    <View style={styles.controlBar}><AnimatedPressable accessibilityLabel="Choose expense group" onPress={() => setPanel(panel === 'group' ? null : 'group')} style={[styles.groupButton, { backgroundColor: colors.card, borderColor: colors.border }]}><Users size={19} color={colors.accent} /><Text numberOfLines={1} style={[styles.groupText, { color: colors.text }]}>{groupId ? groups.find((group) => group.id === groupId)?.name ?? 'Group' : 'Personal'}</Text><ChevronDown size={16} color={colors.textMuted} /></AnimatedPressable></View>
    ...
    {debugError && <View style={[styles.debugErrorBox, { backgroundColor: '#3A1010', borderColor: colors.danger }]}><Text selectable style={styles.debugErrorText}>{debugError}</Text></View>}
