...
export function AddExpenseModal({ visible, groups, expense, onClose, onSaved }: Props) {
  ...
  const submit = async () => {
    ...
    } catch (cause) { fail(cause instanceof Error ? cause.message : 'Unable to save the expense.'); }
  };
  ...
  return <><AppModal visible={visible} onClose={mutation.loading ? () => {} : onClose}><ValidationFeedback trigger={errorPulse}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.content}>
    ...
    <View style={styles.actions}><Button label="Cancel" variant="secondary" disabled={mutation.loading} onPress={onClose} style={styles.action} /><Button label={expense ? 'Save revision' : 'Add expense'} loading={mutation.loading} onPress={() => void submit()} style={styles.action} /></View>
