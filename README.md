...
2. Run the SQL files in `supabase/migrations` in numeric order. Finance uses the clean-slate backend in `010_rebuild_finance.sql`; see [finance architecture](docs/finance-core.md). Migration `009_remove_finance_backend.sql` removes the retired finance objects before the rebuild is applied.
